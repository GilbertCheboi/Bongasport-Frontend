...
export default function BaseballDetail3(props) {
  ...
  const loadComment = () => { 
    fetch(`https://gilscore.azurewebsites.net/api/Baseball/tweetcomments3/${id}/`, {
      method: 'GET',
      headers:{
  ...
  useEffect(() => {
    ...
    const clickedItem = (data) => { 
      props.navigation.navigate('Detail4 Baseball',{data:data}) }
      const clickedProfile = (data) => { 
        props.navigation.navigate('otherprofile',{data:data}) }

    const createLoad =() => {
      const fomdata = new FormData();
      fomdata.append('comment2', id);
      ...
      //data.append('image', image);
        fetch('https://gilscore.azurewebsites.net/api/Baseball/commentweet3/', {
        method: 'POST',
        headers: {
